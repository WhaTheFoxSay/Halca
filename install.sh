...
mkdir -p "$HAIKU_BIN_DIR" 2>/dev/null || true
...
# 1. System Diagnostics (Linux, macOS, FreeBSD, OpenBSD, NetBSD, Haiku OS)
...
if [ "$BINARY_DOWNLOADED" = true ] && [ -s "$CLIENT_BIN" ]; then
    ...
    echo -e "\n    ${YELLOW}[!] Pre-compiled binary mirror missed. Switching to native OS package compiler...${RESET}\n"
fi

# Fallback: Native Package Manager & Source Compiler (Linux, BSD, Haiku OS)
if [ "$BINARY_DOWNLOADED" = false ]; then
    case "$UNAME_OS" in
        freebsd|openbsd|netbsd|dragonfly)
            echo -e "${YELLOW}[+] BSD OS detected. Ensuring compiler and Rust toolchain...${RESET}"
            if command -v pkg &>/dev/null; then
                sudo pkg install -y rust gcc 2>/dev/null || true
                log_installed "DEP" "bsd_pkg_rust"
            elif command -v pkg_add &>/dev/null; then
                sudo pkg_add rust gcc 2>/dev/null || true
                log_installed "DEP" "bsd_pkg_add_rust"
            fi
            ;;
        haiku)
            echo -e "${YELLOW}[+] Haiku OS detected. Ensuring compiler and Rust package...${RESET}"
            if command -v pkgman &>/dev/null; then
                pkgman install -y rust_bin devel:gcc 2>/dev/null || true
                log_installed "DEP" "haiku_pkgman_rust"
            fi
            ;;
        *)
            ;;
...
# Install Executable Binary into all standard BIN directories across Linux, macOS, BSD, and Haiku OS
echo -e "${CYAN}[+] Installing executable binary to system paths...${RESET}"
TARGET_DIRS=("$CARGO_BIN_DIR" "$LOCAL_BIN_DIR" "$USER_BIN_DIR" "$HAIKU_BIN_DIR" "/usr/local/bin")
...
# Configure PATH and Aliases in active Shell RC files (Linux, macOS, BSD, Haiku OS)
SHELL_FILES=(
    "$HOME/.zshrc"
    "$HOME/.bashrc"
    "$HOME/.bash_profile"
...
for RC_FILE in "${SHELL_FILES[@]}"; do
    if [ -f "$RC_FILE" ]; then
        if ! grep -q 'PATH.*\.cargo/bin' "$RC_FILE" 2>/dev/null && ! grep -q 'PATH.*\.local/bin' "$RC_FILE" 2>/dev/null; then
            ...
            echo 'export PATH="$HOME/.cargo/bin:$HOME/.local/bin:$HOME/bin:/boot/home/config/non-packaged/bin:$PATH"' >> "$RC_FILE"
...
export PATH="$CARGO_BIN_DIR:$LOCAL_BIN_DIR:$USER_BIN_DIR:$HAIKU_BIN_DIR:/usr/local/bin:$PATH"
...
echo -e "   ${CYAN}>>> (Jika belum terbaca di terminal saat ini, jalankan: source ~/.bashrc atau source ~/.profile) <<<${RESET}\n"
echo -e "${GREEN}============================================================${RESET}\n"
